...
Klávesy: `mezerník` pauza · `.` krok · `R` náhodná matice · `Shift+R` překvapení · `Z` zmutovat · `Y` souměrná · `I` obrátit · `X` prohodit · `0` vynulovat · `Ctrl+Z/Y` zpět/znovu · `N` rozmístit · `V` rozfoukat · `1–5` nástroje · `[` `]` štětec · `+` `−` rychlost · `←` `→` předchozí/další svět · `B` živá síť · `T` stopy · `L` laboratoř · `G` galerie · `H` skrýt rozhraní · `F` celá obrazovka · `M` zvuk · `S` uložit · `C` obrázek · `Shift+C` video · `U` odkaz · `A` promítání · `?` nápověda.
